...
local function GlobalErrorHandler(err)
    warn("⚠️ Script Error: " .. tostring(err))
    ...
    pcall(function()
        if Library and Library.Notify then
            Library:Notify({
                Title = "⚠️ Error",
                Description = "Terjadi error, cek console (F9)",
                Time = 5
            })
        end
    end)
...
-- ==============================================
-- 1. LOAD LIBRARY (FIXED WITH FALLBACK)
-- ==============================================
...
for attempt = 1, maxAttempts do
    ...
    if success and Library then
        ...
        print("Attempt " .. attempt .. " failed: " .. tostring(err))
        task.wait(1)
    end
end

if not loadSuccess or not Library then
    warn("⚠️ Gagal load library, menggunakan fallback...")
    ...
    Library = {
                ...
                AddTab = function() 
                    return {
                        AddLeftGroupbox = function() 
                            return {
                                AddCheckbox = function(_, data) 
                                    return {
                                        AddColorPicker = function() end,
                                        AddKeyPicker = function() end
                                    }
                                end,
                        ...
                        SetHeaderGlow = function() end,
                        SetProfileVisible = function() end
                    }
                end,
                SetHeaderGlow = function() end,
                SetProfileVisible = function() end
        ...
        Notify = function(_, data)
            print(string.format("[%s] %s", data.Title or "Notify", data.Description or ""))
            task.wait(0.1)
        end,
...
local GotBlindedRemote = nil
local EmoteRemote = nil

SafeCall(function()
    Remotes = ReplicatedStorage:FindFirstChild("Remotes")
    if Remotes then
        AttackEvent = Remotes:FindFirstChild("Attacks") and Remotes.Attacks:FindFirstChild("BasicAttack")
        SkillCheckRemote = Remotes:FindFirstChild("Generator") and Remotes.Generator:FindFirstChild("SkillCheckResultEvent")
        EmoteRemote = Remotes:FindFirstChild("EmoteHandler")
        
        local ToFItems = Remotes:FindFirstChild("Items")
        if ToFItems then
            local ToFFolder = ToFItems:FindFirstChild("Twist of Fate")
            ToFFireRemote = ToFFolder and ToFFolder:FindFirstChild("Fire")
            local FlashlightFolder = ToFItems:FindFirstChild("Flashlight")
            GotBlindedRemote = FlashlightFolder and FlashlightFolder:FindFirstChild("GotBlinded")
        end
    end
end)
...
-- WhatsApp Channel
local WhatsApp = {
    ChannelUrl = "https://whatsapp.com/channel/0029Vb8MsxY7T8bUAjemYH2j",
...
local UpdateInfo = {
    ...
    LastCheck = 0,
    CheckInterval = 3600,
    UpdateUrl = "https://raw.githubusercontent.com/kezodxyz/KezodX/refs/heads/main/version.json"
}
...
-- ==============================================
-- 6. HELPER FUNCTIONS TAMBAHAN
-- ==============================================

local function GetServerList()
    local servers = {}
    local success, result = pcall(function()
        return TeleportService:GetServerListAsync()
    end)
    ...
    warn("⚠️ GetServerListAsync gagal atau kosong, menggunakan server saat ini")
...
local function JoinServer(serverId)
    if ServerHop.IsHoping then
        Library:Notify({
            Title = "Server Hop",
            Description = "Masih dalam proses hop...",
            Time = 2
        })
        return false
    end
    
    ServerHop.IsHoping = true
    local success, err = pcall(function()
        TeleportService:TeleportToServerInstance(game.PlaceId, serverId)
    end)
    ServerHop.IsHoping = false
    
    if not success then
        Library:Notify({
            Title = "Server Hop",
            Description = "Gagal join server: " .. tostring(err),
            Time = 3
        })
        return false
    end
...
local function FindBestServer(region)
    local servers = GetServerList()
    if #servers == 0 then
        Library:Notify({
            Title = "Server Hop",
            Description = "Tidak ada server ditemukan!",
            Time = 3
        })
        return nil
    end
    ...
    if region ~= "All" then
        ...
        for _, server in ipairs(servers) do
            if server.Region and server.Region:find(region, 1, true) then
        ...
        if #filtered == 0 then
            Library:Notify({
                Title = "Server Hop",
                Description = "Tidak ada server dengan region " .. region,
                Time = 3
            })
            return nil
        end
...
local function RejoinServer()
    ...
    if not success then
        pcall(function()
            ...
            if robloxGui then
                local main = robloxGui:FindFirstChild("Main")
                if main then
                    local bottomBar = main:FindFirstChild("BottomBar")
                    if bottomBar then
                        local exit = bottomBar:FindFirstChild("Exit")
                        if exit then
                            local button = exit:FindFirstChild("Button")
                            if button then
                                button:Click()
                            end
                        end
...
local function OpenWhatsAppChannel()
    pcall(function()
        if syn and syn.request then
            syn.request({
                Url = WhatsApp.ChannelUrl,
                Method = "GET"
            })
        end
        setclipboard(WhatsApp.ChannelUrl)
        Library:Notify({
...
local function CreateWhatsAppButton()
    if WhatsApp.ButtonInstance then
        WhatsApp.ButtonInstance:Destroy()
        WhatsApp.ButtonInstance = nil
    end
    
    if not WhatsApp.ShowButton then return end
    
    local gui = Instance.new("ScreenGui")
    gui.Name = "WhatsAppButton"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.Parent = PlayerGui
    WhatsApp.ButtonInstance = gui
    
    local button = Instance.new("ImageButton")
    button.Size = UDim2.new(0, 50, 0, 50)
    button.Position = UDim2.new(0.01, 0, 0.92, 0)
    button.BackgroundTransparency = 1
    button.Image = WhatsApp.Icon
    button.ImageColor3 = Color3.fromRGB(37, 211, 102)
    button.Parent = gui
    ...
    stroke.Color = Color3.fromRGB(37, 211, 102)
    stroke.Thickness = 2
    ...
    tooltip.Position = UDim2.new(0, 55, 0, 12)
    tooltip.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    tooltip.BackgroundTransparency = 0.7
    tooltip.Text = "WhatsApp Channel"
    ...
    button.MouseButton1Click:Connect(OpenWhatsAppChannel)
end

local function SetupAutoRejoin()
    if ServerHop.Enabled then
        local conn
        conn = LocalPlayer.OnTeleport:Connect(function(state)
            if state == Enum.TeleportState.TeleportFailed then
                if ServerHop.Enabled and not ServerHop.IsHoping then
                    task.wait(ServerHop.Delay)
                    local server = FindBestServer(ServerHop.Region)
                    if server then
...
local function CheckForUpdates()
    local now = tick()
    if now - UpdateInfo.LastCheck < UpdateInfo.CheckInterval then return end
    UpdateInfo.LastCheck = now
    
    pcall(function()
        local response = HttpService:GetAsync(UpdateInfo.UpdateUrl)
        local data = HttpService:JSONDecode(response)
        
        if data and data.version and data.version ~= UpdateInfo.Version then
            Library:Notify({
                Title = "📢 Update Tersedia!",
                Description = "Versi " .. data.version .. " tersedia! Kunjungi WhatsApp Channel",
                Time = 5
            })
        end
    end)
...
SafeCall(GB_CreateButton)

LocalPlayer.CharacterAdded:Connect(function()
    task.wait(0.5)
    SafeCall(GB_CreateButton)
    SafeCall(GB_UpdateButton)
end)
...
local PerksFolder = Remotes and Remotes:FindFirstChild("Perks")
local PerfPlanningRemote = PerksFolder and PerksFolder:FindFirstChild("perfectionistplanning")
local GeneratorFolder = Remotes and Remotes:FindFirstChild("Generator")
...
local function setupSilentAimHook()
    ...
    if not castTable then
        Library:Notify({Title="Silent Aim Error", Description="Fungsi 'cast' tidak ditemukan", Time=3})
        pcall(function()
            local oldNamecall
    ...
    Library:Notify({Title="Silent Aim", Description="Hook installed!", Time=2})
end
...
local function removeSilentAimHook()
    ...
    Library:Notify({Title="Silent Aim", Description="Hook removed", Time=2})
end

-- ==============================================
-- 11. TOF SILENT AIM SYSTEM (FIXED)
-- ==============================================
...
local function TeleportToHook()
    ...
    if #hooks == 0 then
        Library:Notify({Title = "TP Hook", Description = "Tidak ada hook!", Time = 2})
        return
    end
...
local function TeleportToGate()
    ...
    if #gates == 0 then
        Library:Notify({Title = "TP Gate", Description = "Tidak ada gate!", Time = 2})
        return
    end
...
local function RefreshMapForTeleport()
    ...
    Library:Notify({Title = "Refresh Map", Description = "Cache diperbarui!", Time = 2})
end
...
-- ==============================================
-- 41. INIT TAMBAHAN (WhatsApp, Server Hop)
-- ==============================================

task.wait(1)
SafeCall(CreateWhatsAppButton)
SafeCall(SetupAutoRejoin)
...
task.spawn(function()
    while task.wait(UpdateInfo.CheckInterval) do
        SafeCall(CheckForUpdates)
    end
end)
...
local Tabs = {
    ...
    Server = Window:AddTab("Server", "server","Server Hop, Rejoin & Updates"),
...
local ServerHopBox = Tabs.Server:AddLeftGroupbox("Server Hop", "server")
local WhatsAppBox = Tabs.Server:AddRightGroupbox("WhatsApp Channel", "message-circle")
...
ESPStatusBox:AddLabel("Teleport Loops")
...
-- ==============================================
-- 44. GUI - PLAYER TAB (Survivor)
-- ==============================================

AbilityTab:AddCheckbox("Skill", {
    Text = "Auto Skill Check", Default = false,
    Callback = function(v) Auto.SkillCheck = v; if v then SafeCall(startSkillCheck) end end
})
...
KillerTab:AddCheckbox("AutoStalk", {
    ...
    Callback = function(v)
        AutoStalk.Enabled = v
        if v then SafeCall(startAutoStalk) else SafeCall(stopAutoStalk) end
    end
})
...
KillerTab:AddCheckbox("BypassCooldown", {
    ...
    Callback = function(state)
        ...
        if state then
            SafeCall(StartCooldownBypass)
            Library:Notify({Title = "Bypass Cooldown", Description = "Diaktifkan", Time = 3})
        else
            SafeCall(StopCooldownBypass)
            Library:Notify({Title = "Bypass Cooldown", Description = "Dinonaktifkan", Time = 3})
        end
    end
})
...
KillerTab:AddCheckbox("BypassLeapCooldown", {
    ...
    Callback = function(v)
        Killer.BypassLeap = v
        if v then SafeCall(StartLeapBypass) end
    end
})
...
KillerTab:AddCheckbox("AttackAim", {
    ...
    Callback = function(v) AttackAim.Enabled = v; if v then SafeCall(startAttackAim) end end
})
...
local FakeParryToggle = ParryBox:AddToggle("FakeParry", {
    ...
    Callback = function(v)
        FakeParry.Enabled = v
        if UserInputService.TouchEnabled then
            if v then SafeCall(CreateFakeParryButton) else SafeCall(RemoveFakeParryButton) end
        end
    end
})
...
local SilentAimToggle = AimlockBox:AddToggle("SilentAimToggle", {
    ...
    Callback = function(v)
        SilentAim.Enabled = v
        if v then
            SafeCall(setupSilentAimHook)
        else
            SafeCall(removeSilentAimHook)
        end
    end
})
...
EmoteBox:AddToggle("ShowEmoteButton", {
    ...
    Callback = function(v)
        EmoteButton.Show = v
        if v then SafeCall(createEmoteButton) else SafeCall(removeEmoteButton) end
    end
})

-- ==============================================
-- 52. GUI - FUN TAB (Fake Chat Tag)
-- ==============================================
...
-- Server Hop
ServerHopBox:AddToggle("ServerHopEnabled", {
    Text = "Enable Auto Server Hop",
    Default = false,
    Tooltip = "Otomatis mencari server baru saat keluar",
    Callback = function(v)
        ServerHop.Enabled = v
        if v then
            SafeCall(SetupAutoRejoin)
...
ServerHopBox:AddButton({
    Text = "Find Best Server",
    Func = function()
        local server = FindBestServer(ServerHop.Region)
        if server then
            Library:Notify({
                Title = "Server Found",
                Description = "Players: " .. (server.PlayerCount or 0) .. " | Region: " .. (server.Region or "Unknown"),
                Time = 3
            })
        end
    end
...
ServerHopBox:AddButton({
    Text = "Join Best Server",
    Func = function()
        local server = FindBestServer(ServerHop.Region)
        if server then
            SafeCall(JoinServer, server.Id)
        end
    end
})
...
ServerHopBox:AddButton({
    ...
    Func = function()
        SafeCall(RejoinServer)
    end
})

ServerHopBox:AddDropdown("ServerRegion", {
    Values = {"All", "US", "EU", "Asia", "Australia", "South America"},
    Default = 1,
    Text = "Region Filter",
    Callback = function(v) ServerHop.Region = v end
})

ServerHopBox:AddSlider("HopDelay", {
    Text = "Hop Delay (seconds)",
    Default = 5,
    Min = 1,
    Max = 30,
    Rounding = 0,
    Callback = function(v) ServerHop.Delay = v end
})

-- WhatsApp Channel
WhatsAppBox:AddButton({
...
WhatsAppBox:AddButton({
    ...
    Func = function()
        ...
        Library:Notify({
            Title = "Copied!",
            Description = "Link WhatsApp Channel disalin!",
            Time = 2
        })
    end
})

WhatsAppBox:AddToggle("ShowWhatsAppButton", {
    Text = "Show WhatsApp Button",
    Default = true,
    Tooltip = "Tampilkan tombol WhatsApp di layar",
    Callback = function(v)
        WhatsApp.ShowButton = v
        if v then
            SafeCall(CreateWhatsAppButton)
        else
            if WhatsApp.ButtonInstance then
...
WhatsAppBox:AddDivider()
WhatsAppBox:AddLabel("📢 Version: " .. UpdateInfo.Version)
...
WhatsAppBox:AddButton({
    ...
    Func = function()
        ...
        Library:Notify({
            Title = "Update Check",
            Description = "Sedang mengecek update...",
            Time = 2
        })
    end
})
...
SettingBox:AddKeyPicker("RejoinKeybind", {
    Default = "None",
    Text = "Rejoin Server",
    Mode = "Toggle",
    Callback = function()
...
SettingBox:AddKeyPicker("HopServerKeybind", {
    Default = "None",
    Text = "Hop Server",
    Mode = "Toggle",
    Callback = function()
...
Library:Notify({
    Title = "GanKunZ Hub v3.1",
    Description = "Loaded! No Key Required!",
    Time = 3
})

Library:Notify({
    Title = "📱 WhatsApp Channel",
    Description = "Klik tombol WhatsApp di layar atau tab Server",
    Time = 4
})

print("✅ GanKunZ Hub v3.1 Loaded!")
print("📌 All Features Ready!")
print("🎯 Menu Key: RightShift")
print("📱 WhatsApp Channel: https://whatsapp.com/channel/0029Vb8MsxY7T8bUAjemYH2j")
