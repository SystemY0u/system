...
local Window = Library:CreateWindow({
    Title = "GanKunZ Hub",
    Footer = 'Full Features | by GanKunZ | 📱 WhatsApp Channel',
    Icon = "1462874790",
...
-- ==============================================
-- 56. WHATSAPP CHANNEL SECTION
-- ==============================================

SettingBox:AddDivider()
SettingBox:AddLabel("WhatsApp Channel")

local WhatsAppToggle = SettingBox:AddToggle("ShowWhatsAppButton", {
    Text = "Show WhatsApp Button",
    Default = true,
    Callback = function(Value)
        State.WhatsAppVisible = Value
        if State.WhatsAppButton then
            State.WhatsAppButton.Enabled = Value
        end
    end
})
...
SettingBox:AddButton({
    ...
    Func = function()
        local channelUrl = "https://whatsapp.com/channel/0029Vb8MsxY7T8bUAjemYH2j"
        pcall(function()
            setclipboard(channelUrl)
            Library:Notify({
        ...
        pcall(function()
            GuiService:OpenBrowserWindow(channelUrl)
        end)
    end
})
...
-- ==============================================
-- 57. WHATSAPP BUTTON CREATION
-- ==============================================

local function CreateWhatsAppButton()
    if State.WhatsAppButton then
        pcall(function() State.WhatsAppButton:Destroy() end)
        State.WhatsAppButton = nil
    end
    
    local whatsappGui = Instance.new("ScreenGui")
    whatsappGui.Name = "WhatsAppChannelGui"
    whatsappGui.ResetOnSpawn = false
    whatsappGui.IgnoreGuiInset = true
    whatsappGui.Parent = PlayerGui
    
    local whatsappBtn = Instance.new("ImageButton")
    whatsappBtn.Name = "WhatsAppButton"
    whatsappBtn.Size = UDim2.new(0, 55, 0, 55)
    whatsappBtn.Position = UDim2.new(0.01, 0, 0.92, 0)
    whatsappBtn.AnchorPoint = Vector2.new(0, 1)
    whatsappBtn.BackgroundColor3 = Color3.fromRGB(37, 211, 102)
    whatsappBtn.BackgroundTransparency = 0.15
    whatsappBtn.Image = "15960010296"
    whatsappBtn.ImageColor3 = Color3.fromRGB(255, 255, 255)
    whatsappBtn.ImageTransparency = 0.1
    whatsappBtn.AutoButtonColor = true
    whatsappBtn.ZIndex = 10
    whatsappBtn.Parent = whatsappGui
    
    local corner = Instance.new("UICorner")
    corner.CornerRadius = UDim.new(1, 0)
    corner.Parent = whatsappBtn
    
    local stroke = Instance.new("UIStroke")
    stroke.Color = Color3.fromRGB(255, 255, 255)
    stroke.Thickness = 2
    stroke.Transparency = 0.3
    stroke.Parent = whatsappBtn
    
    local tooltip = Instance.new("TextLabel")
    tooltip.Size = UDim2.new(0, 160, 0, 30)
    tooltip.Position = UDim2.new(0, 60, 0, 12)
    tooltip.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    tooltip.BackgroundTransparency = 0.2
    tooltip.Text = "Join WhatsApp Channel"
    tooltip.TextColor3 = Color3.fromRGB(255, 255, 255)
    tooltip.TextSize = 12
    tooltip.Font = Enum.Font.GothamBold
    tooltip.Visible = false
    tooltip.ZIndex = 11
    tooltip.Parent = whatsappBtn
    
    local tooltipCorner = Instance.new("UICorner")
    tooltipCorner.CornerRadius = UDim.new(0, 5)
    tooltipCorner.Parent = tooltip
    
    whatsappBtn.MouseEnter:Connect(function()
        tooltip.Visible = true
    end)
    
    whatsappBtn.MouseLeave:Connect(function()
        tooltip.Visible = false
    end)
    ...
    State.WhatsAppButton = whatsappGui
    
    if not State.WhatsAppVisible then
        whatsappGui.Enabled = false
    end
end

-- Buat WhatsApp button
task.spawn(function()
    task.wait(2)
    CreateWhatsAppButton()
end)

-- Recreate saat karakter spawn
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(2)
    if State.WhatsAppButton then
        pcall(function() State.WhatsAppButton:Destroy() end)
